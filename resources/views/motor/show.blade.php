    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-700 dark:text-gray-300">
            <div class="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                ...
                <span>Tahun: {{ $motor->tahun_motor }}</span>
            </div>
            <div class="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                <svg class="w-6 h-6 text-green-500" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M4 6h16v2H4zm0 5h10v2H4zm0 5h7v2H4z"/>
                </svg>
                <span>Jarak Tempuh: {{ number_format($motor->km_motor, 0, ',', '.') }} km</span>
            </div>
        </div>
        ...
        <div class="mt-6">
            <a 
                href="https://wa.me/628123456789?text=Halo%2C%20saya%20ingin%20memesan%20{{ urlencode($motor->nama_motor) }}" 
                target="_blank" 
        ...
        <form id="delete-form-{{ $motor->id }}" action="{{ route('motor.destroy', $motor->id) }}" method="POST" class="flex-1">
            @csrf
            @method('DELETE')
            <button type="button" onclick="confirmDelete({{ $motor->id }})" 
...
function confirmDelete(id) {
    Swal.fire({
        title: 'Yakin ingin menghapus motor ini?',
